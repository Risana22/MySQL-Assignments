...
UPDATE employees
SET designation='Data scientist' WHERE designation IS NULL;
...
SELECT * FROM employees
 ...
 ORDER BY YEAR(hire_date)=2018 ASC
 LIMIT 5; 
 
SELECT SUM(salary) AS sum_salary_finance
 FROM employees
 GROUP BY department_id
 HAVING department_id=7;
 
SELECT MIN(age) AS Minimum_age 
 FROM employees;
 
 SELECT e.location_id, l.location,
 MAX(salary) AS Maximum_salary
 FROM employees e left join location l on e.location_id=l.location_id
...
SELECT designation, AVG(salary) AS Average_salary
 ...
 select e.department_id,d.department_name, count(*) as no_of_employees
 from employees e left join departments d on e.department_id=d.department_id
 group by department_id having count(*)<3;
 
SELECT e.Location_id, l.location,ROUND(AVG(age)) AS average_age
FROM employees e LEFT JOIN location l on e.location_id=l.location_id
WHERE gender='F'
GROUP BY location_id
HAVING average_age<30;
 ...
 SELECT e.location_id,l.location,
 IFNULL(e.employee_name,'NULL') AS employee_name
